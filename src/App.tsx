      ...
      {state.data && (
        <aside className="fixed top-16 right-0 bottom-0 z-10 w-80 p-6 overflow-y-auto bg-white/5 backdrop-blur-lg border-l border-white/10">
          <div className="space-y-6">
                  ...
                  <div className="space-y-4">
                    <div>
                      <label className="block text-sm mb-2 text-gray-600">
                        Highlight Depth: {state.viewState.highlightDepth}
                      </label>
                      <input
                        type="range"
                        min="1"
                        max="5"
                        value={state.viewState.highlightDepth}
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <button
                        className="p-3 rounded-xl bg-gradient-to-br from-gray-50 to-gray-200 shadow-neumorphic-raised hover:shadow-neumorphic-pressed active:shadow-neumorphic-pressed transition-all duration-200 flex items-center justify-center group"
                        onClick={() => {
                ...
                {state.viewState.selectedNode && (() => {
                  ...
                  return (
                    <div>
                      ...
                      <div className="p-4 rounded-xl bg-green-50/30 backdrop-blur-sm shadow-neumorphic-inset text-sm border border-green-100/20">
                        <div className="font-medium text-gray-700 mb-2">
                          {node?.path || state.viewState.selectedNode}
                        </div>
                      ...
                      <div className="p-4 rounded-xl bg-green-50/30 backdrop-blur-sm shadow-neumorphic-inset text-sm border border-green-100/20">
                        ...
                        <div className="text-xs text-gray-600 space-y-1">
                          <div className="truncate">
                            From: {sourceNode?.path?.split('/').pop() || sourceId}
                          </div>
                          <div className="truncate">
                            To: {targetNode?.path?.split('/').pop() || targetId}
                          </div>
                          {link.imports && link.imports.length > 0 && (
                            <div className="mt-2">
                              Imports: {link.imports.join(', ')}
                            </div>
                          )}
                        </div>
                      </div>
                    ...
                    <div className="space-y-2 max-h-40 overflow-y-auto">
                      {Array.from(state.viewState.connectedEntities).map(entityId => {
                        ...
                        return (
                          <div 
                            key={entityId} 
                            className="p-3 rounded-xl bg-blue-50/20 backdrop-blur-sm shadow-neumorphic-inset text-sm border border-blue-100/15 cursor-pointer hover:bg-blue-50/30 transition-colors duration-200"
                ...
                <div className="text-xs space-y-1.5 pt-4 text-gray-600">
                  <div>• Click node/link to select</div>
                  <div>• Double-click node to center</div>
