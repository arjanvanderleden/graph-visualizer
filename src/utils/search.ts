...
export const searchGraphEntities = (
  data: GraphData, 
  searchTerm: string,
  ...
  data.nodes.forEach(node => {
    NODE_SEARCH_PROPERTIES.forEach(prop => {
      ...
      if (value && typeof value === 'string' && value.toLowerCase().includes(term)) {
        const displayText = node.path || node.id || node.name || String(node.id);
        results.push({
          type: 'node',
          id: node.id,
  ...
  data.links.forEach(link => {
    ...
    LINK_SEARCH_PROPERTIES.forEach(prop => {
      ...
      if (prop === 'imports' && Array.isArray(value)) {
        ...
        const matchingImports = value.filter(imp => 
          typeof imp === 'string' && imp.toLowerCase().includes(term)
        );
